...
from src.rapid.build_db import build_db, build_normal_db, distance, get_core_set

def predict_test(test_X, normal_embedding_db):
    test_log_seqs = test_X.to_list()
    db = build_db(test_log_seqs)

    ## INFERENCE
    abnormal_scores = [0] * db.num_unique_logs
    for index in range(db.num_unique_logs):
        core_set = get_core_set(db.embedding_db[index][0])
        abnormal_scores[index] = min(distance(db.embedding_db[index], normal_embedding_db[c_index]) for c_index in core_set[1:])
...
class Rapid:
    ...
    def fit(self, normal_logs) -> None:
        self.normal_db, self.knn = build_normal_db(normal_logs)

    def predict(self, test_logs) -> pd.Series:
        return predict_test(test_logs, self.normal_db)