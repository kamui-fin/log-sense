...
class DBCache:
    ...
    def cache_exists(self):
        return all(p.exists() for p in [self.embedding_db_path, self.index_to_unique_path, self.lookup_db_path])
...
def build_db(log_sequences: List[str], db_key: str, output_dir: Path):
    ...
    embedding_db = torch.zeros((num_unique_logs, model.config.max_position_embeddings, model.config.hidden_size))
...
def get_core_set(query, knn):
    return knn.k_nearest_neighbors.kneighbors(query.detach().numpy().reshape(1, -1))[-1][0]
