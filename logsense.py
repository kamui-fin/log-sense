...
class LogGPT:
    def __init__(self, model_path, vocab_size):
        ...
        self.model, self.optimizer = setup_model_optimizer(self.vocab_size, cache_path=self.model_path)
    
    def pre_train(self, normal_logs):
        self.model, self.optimizer, pretrain_losses = pretrain_model(normal_logs, self.vocab_size, output_path=self.model_path)
        return pretrain_losses

    def finetune(self, normal_logs) -> None:
        finetune_losses = finetune(self.model, self.optimizer, normal_logs, cache_path=self.model_path, sliding_window=True)
