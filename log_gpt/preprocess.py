...
def load_bgl(bgl_data_dir, cache_file: Path):
    ...
    df['event_id'] = df['line'].apply(lambda line: template_miner.match(line).cluster_id + num_special_tokens)
    windows = [(t, part) for t, part in df.groupby(pd.Grouper(key='timestamp', freq='1Min')) if len(part)]
    ...
    for timestamp, one_min_window in windows:
        ...
        chunks = [one_min_window.iloc[i * context_size: (i + 1) * context_size] for i in range(splits)]
