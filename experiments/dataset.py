...
class Hdfs:
    def __init__(
        self, file_path: Path, labels_path: Path, output_path: Path, drain_mode=True
    ):
    ...
    def add_labels(self, labels_path: Path):
        ...
        self.df = pd.merge(
            self.df, true_labels, left_on="block_id", right_on="BlockId"
        ).rename(columns={"Label": "is_anomaly"})
    ...
    def train_test_split(self, train_size=446_578):
        ...
        test = pd.concat(
            [df[df["is_anomaly"] == 0].iloc[train_size:], df[df["is_anomaly"] == 1]]
        )
        test_X = test["line"]
...
class BGL:
    ...
    def regex_sub_line(self, line: str):
        ...
        timestamp = datetime.strptime(
            re.findall(date_time_regex, line)[0], "%Y-%m-%d-%H.%M.%S.%f"
        )
    ...
    def clean_parse_dataset(self):
        ...
        for line in tqdm(self.data):
            ...
            cleaned_logs.append(line)
        return pd.DataFrame(
            {"line": cleaned_logs, "timestamp": timestamps, "is_anomaly": labels}
        )
    ...
    def train_test_split(
        self, train_size=3_519_602, test_normal_size=879_910, test_abnormal_size=348_460
    ):
        df = self.df
        train = df[df["is_anomaly"] == 0].iloc[:train_size]
        train_X = train["line"]
        ...
        test = pd.concat(
            [
                df[df["is_anomaly"] == 0].iloc[
                    train_size : train_size + test_normal_size
                ],
                df[df["is_anomaly"] == 1].iloc[:test_abnormal_size],
            ]
        )
        test_X = test["line"]
        test_y = test["is_anomaly"]
...
class ThunderBird:
    ...
    def regex_sub_line(self, line: str):
        ...
        date_time_str = (
            re.findall(date_regex, line)[0] + " " + re.findall(time_regex, line)[0]
        )
    ...
    def train_test_split(
        self, train_size=3_938_483, test_normal_size=984_621, test_abnormal_size=76_895
    ):
        df = self.df
        train = df[df["is_anomaly"] == 0].iloc[:train_size]
        train_X = train["line"]
