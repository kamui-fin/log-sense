...
def predict_test(test_X, normal_db, output_dir, knn):
    ...
    for index in tqdm(range(db.num_unique_logs)):
        ...
        abnormal_scores[index] = min(
            distance(db.embedding_db[index], normal_db.embedding_db[c_index])
            for c_index in core_set[1:]
        )
...
def evaluate(test_y, final_pred):
    ...
    print(
        classification_report(
            test_y, final_pred > thresholds[ix], target_names=["Normal", "Anomaly"]
        )
    )
